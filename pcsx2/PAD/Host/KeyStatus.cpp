...
KeyStatus::KeyStatus()
{
	...
	for (u32 pad = 0; pad < NUM_CONTROLLER_PORTS; pad++)
	{
		...
		m_pressure_modifier[pad][0] = 0.5f;
		m_pressure_modifier[pad][1] = 0.5f;
		m_pressure_modifier[pad][2] = 0.5f;
	}
}
...
#undef MERGE
	...
	else
	{
		...
		const float pmod = ((m_button[pad] & (1u << PAD_PRESSURE1)) == 0 && !IsTriggerKey(index)) ? m_pressure_modifier[pad][0] : 1.0f;
		...
		// Adjust pressure of all other face buttons which are active when a pressure modifier is pressed..
		float temp_adjust_pmod = 0;
		if (index == PAD_PRESSURE1)
			temp_adjust_pmod = ((m_button[pad] & (1u << PAD_PRESSURE1)) == 0) ? m_pressure_modifier[pad][0] : (1.0f / m_pressure_modifier[pad][0]);
		else if (index == PAD_PRESSURE2)
			temp_adjust_pmod = ((m_button[pad] & (1u << PAD_PRESSURE2)) == 0) ? m_pressure_modifier[pad][1] : (1.0f / m_pressure_modifier[pad][1]);
		else if (index == PAD_PRESSURE3)
			temp_adjust_pmod = ((m_button[pad] & (1u << PAD_PRESSURE2)) == 0) ? m_pressure_modifier[pad][2] : (1.0f / m_pressure_modifier[pad][2]);
		else return;

		const float adjust_pmod = temp_adjust_pmod;
